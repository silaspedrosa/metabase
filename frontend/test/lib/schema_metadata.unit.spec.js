...
describe("schema_metadata", () => {
    describe("getFieldType", () => {
        it("should know a date", () => {
            ...
            expect(getFieldType({ base_type: TYPE.DateTime })).toEqual(
                DATE_TIME
            );
            expect(getFieldType({ base_type: TYPE.Time })).toEqual(DATE_TIME);
            expect(
                getFieldType({ special_type: TYPE.UNIXTimestampSeconds })
            ).toEqual(DATE_TIME);
            expect(
        ...
        it("should know a number", () => {
            expect(getFieldType({ base_type: TYPE.BigInteger })).toEqual(
                NUMBER
            );
        ...
        it("should know a location", () => {
            ...
            expect(getFieldType({ special_type: TYPE.Country })).toEqual(
                LOCATION
            );
        });
        it("should know a coordinate", () => {
            expect(getFieldType({ special_type: TYPE.Latitude })).toEqual(
                COORDINATE
            );
        ...
        it("should know something that is string-like", () => {
            expect(getFieldType({ base_type: TYPE.TextLike })).toEqual(
                STRING_LIKE
            );
        ...
        it("should know what it doesn't know", () => {
            expect(getFieldType({ base_type: "DERP DERP DERP" })).toEqual(
                undefined
            );
        });
    });
