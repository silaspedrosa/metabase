...
describe("admin/people", () => {
    ...
    describe("user management", () => {
        it("should allow admin to create new users", async () => {
            ...
            setInputValue(
                addUserModal.find("input[name='firstName']"),
                firstName
            );
            setInputValue(
                addUserModal.find("input[name='lastName']"),
                lastName
            );
            ...
            createdUserId = Math.max.apply(
                null,
                userIds.map(key => parseInt(key))
            );
            ...
            const userRow = usersTable
                .find(`td[children="${email}"]`)
                .closest("tr");
            expect(
                userRow
                    .find("td")
            ...
            setInputValue(
                editDetailsModal.find("input[name='email']"),
                email + "x"
            );
            ...
            click(userRow.find(UserActionsSelect));
            click(
                app.find(".TestPopover").find('li[children="Reset Password"]')
            );
