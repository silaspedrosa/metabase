...
describe("admin/datamodel", () => {
    ...
    describe("data model editor", () => {
        it("should allow admin to edit data model", async () => {
            ...
            click(
                columnsListItems.first().find(".TableEditor-field-visibility")
            );
            ...
            click(
                columnsListItems.first().find(".TableEditor-field-special-type")
            );
        ...
        it("should allow admin to create segments", async () => {
            ...
            expect(store.getPath()).toBe(
                "/admin/datamodel/segment/create?table=2"
            );
            ...
            setInputValue(
                filterPopover.find("textarea.border-purple"),
                "gmail"
            );
        ...
        it("should allow admin to create metrics", async () => {
            ...
            expect(store.getPath()).toBe(
                "/admin/datamodel/metric/create?table=2"
            );
            await store.waitForActions([
                FETCH_TABLE_METADATA,
                UPDATE_PREVIEW_SUMMARY
            ...
            click(
                app
                    .find("#AggregationPopover")
                    .find('h4[children="Count of rows"]')
            );
