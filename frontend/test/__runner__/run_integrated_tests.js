...
const login = async (apiHost, user) => {
    ...
    if (result.status >= 200 && result.status <= 299) {
        console.log(
            `Successfully created a shared login with id ${resultBody.id}`
        );
        return resultBody;
    } else {
...
const init = async () => {
    ...
    try {
        ...
        console.log(
            chalk.bold("Running integrated test runner with this build:")
        );
    ...
    console.log(
        chalk.bold("3/4 Creating a shared login session for backend 1")
    );
