...
export function downgradeNativePermissionsIfNeeded(
    ...
    let currentNative = getNativePermission(permissions, groupId, {
        databaseId
    });
...
function inferEntityPermissionValueFromChildTables(
    ...
    const entityIdsForDescendantTables: TableEntityId[] = _.chain(
        database.tables
    )
    ...
    for (const table of database.tables) {
        ...
        if (oldFieldsPerm !== newFieldsPerm) {
            if (newFieldsPerm === "none") {
                databaseDiff.revokedTables[table.id] = {
                    name: table.display_name
                };
            } else {
                databaseDiff.grantedTables[table.id] = {
                    name: table.display_name
                };
            }
        }
