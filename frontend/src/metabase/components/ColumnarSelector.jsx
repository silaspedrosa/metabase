                ...
                sectionElements = sections.map((section, sectionIndex) => {
                    ...
                    var items = section.items.map((item, rowIndex) => {
                        var itemClasses = cx({
                            ...
                            "ColumnarSelector-row--selected": isItemSelected(
                                item,
                                column
                            ),
                            "ColumnarSelector-row--disabled": isItemDisabled(
                                item,
                                column
                            ),
                            flex: true,
                        ...
                        var checkIcon = lastColumn ? (
                            <Icon name="check" size={14} />
                        ) : null;
                        var descriptionElement;
                        var description =
                            column.itemDescriptionFn &&
                            column.itemDescriptionFn(item);
                        if (description) {
                            descriptionElement = (
                    ...
                    if (title) {
                        titleElement = (
                            <div className="ColumnarSelector-title">
                                {title}
                            </div>
                        );
                    }
                    return (
                        <section
                            key={sectionIndex}
                            className="ColumnarSelector-section"
                        >
                            {titleElement}
