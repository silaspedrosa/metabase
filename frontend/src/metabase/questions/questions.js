            ...
            try {
                ...
                if (entityType === "cards") {
                    result = {
                        ...
                        ...normalize(
                            momentifyArraysTimestamps(
                                await CardApi.list(entityQueryObject)
                            ),
                            [card]
                        )
                    };
    ...
    (cardId, labelId, labeled, undoable = false) => {
        return async (dispatch, getState) => {
            if (cardId == null) {
                ...
                selected.map(item =>
                    dispatch(setLabeled(item.id, labelId, labeled))
                );
                // TODO: errors
                if (undoable) {
                    dispatch(
                        addUndo(
                            createUndo(
                                ...
                                selected.map(item =>
                                    setLabeled(item.id, labelId, !labeled)
                                )
                            )
                        )
                    );
                        ...
                        addUndo(
                            createUndo(
                                ...
                                selected.map(item =>
                                    setCollection(
                                        item.id,
                                        getCardCollectionId(state, item.id)
                                    )
                                )
                            )
                        )
                    ...
                    dispatch(
                        addUndo(
                            createUndo("moved", [
                                setCollection(
                                    cardId,
                                    getCardCollectionId(state, cardId)
                                )
                            ])
                        )
                    );
    ...
    switch (type) {
        ...
        case SET_COLLECTION_ARCHIVED:
            if (error) {
                ...
                state = assocIn(
                    state,
                    ["entities", "collections", payload.id],
                    {
                        ...getIn(state, [
                            "entities",
                            "collections",
    ...
    return updateIn(state, ["itemsBySection", entityType], entityQueries =>
        _.mapObject(entityQueries, (entityQueryResult, entityQuery) => {
            if (sectionPredicate(JSON.parse(entityQuery))) {
                const doesContain = _.contains(
                    entityQueryResult.items,
                    entityId
                );
                if (!doesContain && shouldContain) {
                    ...
                    return {
                        ...entityQueryResult,
                        items: entityQueryResult.items.filter(
                            id => id !== entityId
                        )
                    };
                }
