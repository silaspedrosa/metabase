    ...
    return async function(dispatch, getState) {
        if (database.details["let-user-control-scheduling"]) {
            try {
                ...
                const { valid } = await MetabaseApi.db_validate({
                    details: database
                });
...
export const syncDatabaseSchema = createThunkAction(
    ...
    function(databaseId) {
        return async function(dispatch, getState) {
            try {
                let call = await MetabaseApi.db_sync_schema({
                    dbId: databaseId
                });
...
export const rescanDatabaseFields = createThunkAction(
    ...
    function(databaseId) {
        return async function(dispatch, getState) {
            try {
                let call = await MetabaseApi.db_rescan_values({
                    dbId: databaseId
                });
                MetabaseAnalytics.trackEvent("Databases", "Manual Sync");
                return call;
...
export const discardSavedFieldValues = createThunkAction(
    ...
    function(databaseId) {
        return async function(dispatch, getState) {
            try {
                let call = await MetabaseApi.db_discard_values({
                    dbId: databaseId
                });
                MetabaseAnalytics.trackEvent("Databases", "Manual Sync");
                return call;
