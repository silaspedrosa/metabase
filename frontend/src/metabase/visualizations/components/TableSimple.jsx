...
export default class TableSimple extends Component {
    ...
    componentDidUpdate() {
        ...
        let footerHeight = this.refs.footer
            ? ReactDOM.findDOMNode(this.refs.footer).getBoundingClientRect()
                  .height
            : 0;
        let rowHeight =
            ReactDOM.findDOMNode(this.refs.firstRow).getBoundingClientRect()
                .height + 1;
        let pageSize = Math.max(
            1,
            Math.floor(
                (this.props.height - headerHeight - footerHeight) / rowHeight
            )
        );
    ...
    render() {
        ...
        if (sortColumn != null) {
            rowIndexes = _.sortBy(
                rowIndexes,
                rowIndex => rows[rowIndex][sortColumn]
            );
            if (sortDescending) {
                rowIndexes.reverse();
            }
        ...
        return (
            ...
            >
                <div className="flex-full relative">
                    <div
                        className="absolute top bottom left right scroll-x scroll-show scroll-show--horizontal scroll-show--hover"
                ...
                {pageSize < rows.length ? (
                    ...
                    >
                        <span className="text-bold">{t`Rows ${start + 1}-${end +
                            1} of ${rows.length}`}</span>
                        <span
                            className={cx(
