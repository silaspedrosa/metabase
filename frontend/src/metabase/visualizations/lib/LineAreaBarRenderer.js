    ...
    const groups = [
        datas.map((data, seriesIndex) =>
            reduceGroup(dimension.group(), seriesIndex + 1, () =>
                warn(
                    UNAGGREGATED_DATA_WARNING(
                        props.series[seriesIndex].data.cols[0]
                    )
                )
            )
        )
        ...
        return data[0]
            ...
            .map((_, metricIndex) =>
                reduceGroup(dim.group(), metricIndex + 1, () =>
                    warn(
                        UNAGGREGATED_DATA_WARNING(
                            series[seriesIndex].data.cols[0]
                        )
                    )
                )
            );
...
function getYAxisProps(props, groups, datas) {
    const yExtents = groups.map(group =>
        d3.extent(group[0].all(), d => d.value)
    );
...
function applyChartLineBarSettings(chart, settings, chartType) {
    ...
    if (chart.interpolate)
        chart.interpolate(
            settings["line.interpolate"] || DEFAULT_INTERPOLATION
        );
...
function getCharts(
    ...
    return groups.map((group, index) => {
        ...
        chart
            ...
            .useRightYAxis(
                yAxisSplit.length > 1 && yAxisSplit[1].includes(index)
            );
        ...
        if (barCharts.length > 0) {
            ...
            barCharts.forEach((barChart, index) => {
                barChart.setAttribute(
                    "transform",
                    "translate(" +
                        (newBarWidth + seriesPadding) * index +
                        ", 0)"
                );
            });
        }
